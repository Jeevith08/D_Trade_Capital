...
def main():
    file_path = 'lib/screens/home_page.dart'
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. State class definition
    old_def = """class TradingView extends StatelessWidget {
  const TradingView({super.key});"""
    new_def = """class TradingView extends StatefulWidget {
  const TradingView({super.key});
...
class _TradingViewState extends State<TradingView> {
  ...
  void _closePosition(Map<String, dynamic> pos) {
    ...
    # handle literal linebreaks since we read from file.
    import platform
    content = content.replace("class TradingView extends StatelessWidget {\r\n  const TradingView({super.key});", new_def)
    content = content.replace("class TradingView extends StatelessWidget {\n  const TradingView({super.key});", new_def)
    

    # 2. Add Account replacement
    old_acc = """            child: Column(
              children: [
                const Row(
    ...
    new_acc = """            child: _isPaperConnected
                ? Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Row(
    ...
    content = content.replace(old_acc.replace('\n', '\r\n'), new_acc)
    
    # 3. Modify Bottom Tabs in _buildChartSection
    old_tabs = """                    Column(
                      children: [
                        Row(
                          children: [
    ...
    new_tabs = """                    _bottomTab('OPEN POSITIONS', 0, badge: _openPositions.length.toString()),
                    const SizedBox(width: 24),
                    _bottomTab('HISTORY', 1),
                    const SizedBox(width: 24),
                    _bottomTab('PENDING ORDERS', 2),"""
    content = content.replace(old_tabs, new_tabs)
    content = content.replace(old_tabs.replace('\n', '\r\n'), new_tabs)

    # 4. _buildPositionsList addition
    old_pl = """          const SizedBox(height: 32),
    ...
    new_pl = """          _buildPositionsList(),"""
    content = content.replace(old_pl, new_pl)
    content = content.replace(old_pl.replace('\n', '\r\n'), new_pl)

    # 5. Execute Trade Buy/Sell Buttons
    old_bts = """              Expanded(
                child: Container(
    ...
    content = content.replace(old_bts, new_bts)
    content = content.replace(old_bts.replace('\n', '\r\n'), new_bts)

    # 6. Lot Size State replacement
    old_lot = """          const Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
    ...
    content = content.replace(old_lot.replace('\n', '\r\n'), new_lot)

    # 7. _lotSizeBox function
    old_box = """  Widget _lotSizeBox(String size, bool active) {
    return Container(
      width: 60,
    ...
    new_box = """  Widget _lotSizeBox(String size) {
    ...
    content = content.replace(old_box.replace('\n', '\r\n'), new_box)

    # 8. Add methods + close state tag
    old_end = """// ---------------------------------------------------------
// INTELLIGENCE VIEW
// ---------------------------------------------------------"""
    new_end = """  Widget _bottomTab(String title, int index, {String? badge}) {
...
// ---------------------------------------------------------"""
    ...
    content = content.replace(old_end.replace('\n', '\r\n'), new_end)
