...
idx_trade = text.find('class TradingView')
idx_intel = text.find('class IntelligenceView')

top_part = text[:idx_trade]
bottom_part = text[idx_intel:]

trading_view_code = """class TradingView extends StatefulWidget {
...
final_content = top_part + trading_view_code + bottom_part # bottom_part has '// INTELL...' so bottom_part doesn't matter, we just use bottom_part

with open('lib/screens/home_page.dart', 'w', encoding='utf-8') as f:
    f.write(top_part + trading_view_code + bottom_part)

print("Absolute overwrite done.")
